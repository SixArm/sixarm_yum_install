...
sudo yum install -y lua5.2                 `# Lua - small embeddable language with simple procedural syntax`
...
## Haskell
sudo yum install -y libghc-cgi-dev
sudo yum install -y libghc-fgl-dev
sudo yum install -y libghc-glut-dev
sudo yum install -y libghc-haskell-src-dev
sudo yum install -y libghc-html-dev libghc-http-dev
sudo yum install -y libghc-hunit-dev libghc-mtl-dev
sudo yum install -y libghc-network-dev
sudo yum install -y libghc-opengl-dev
sudo yum install -y libghc-parallel-dev
sudo yum install -y libghc-parsec3-dev
sudo yum install -y libghc-quickcheck2-dev
sudo yum install -y libghc-regex-base-dev
sudo yum install -y libghc-regex-compat-dev
sudo yum install -y libghc-regex-posix-dev
sudo yum install -y libghc-stm-dev
sudo yum install -y libghc-syb-dev
sudo yum install -y libghc-text-dev
sudo yum install -y libghc-transformers-dev
sudo yum install -y libghc-xhtml-dev
sudo yum install -y libghc-zlib-dev
sudo yum install -y ghc                    `# The Glasgow Haskell Compilation system`
sudo yum install -y alex                   `# lexical analyser generator for Haskell`
sudo yum install -y cabal-install          `# command-line interface for Cabal and Hackage`
sudo yum install -y happy                  `# Parser generator foh Haskell`
